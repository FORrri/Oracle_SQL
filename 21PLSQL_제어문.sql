...
BEGIN
    ...
    */
    ...
    CASE WHEN POINT >= 90 THEN DBMS_OUTPUT.PUT_LINE('A학점 입니다');
         WHEN POINT >= 90 THEN DBMS_OUTPUT.PUT_LINE('A학점 입니다');
         WHEN POINT >= 90 THEN DBMS_OUTPUT.PUT_LINE('A학점 입니다');
         WHEN POINT >= 90 THEN DBMS_OUTPUT.PUT_LINE('A학점 입니다');
...
BEGIN
    WHILE CNT <= 9
    LOOP
        DBMS_OUTPUT.PUT_LINE('3X' || CNT || '=' || CNT * 3);
...
BEGIN
    FOR I IN 1..9 -- 1~9까지
    LOOP
        CONTINUE WHEN I = 5; --I가 5면 다음으로(I가 5면 출력 안됨)
        DBMS_OUTPUT.PUT_LINE('3X'|| I || '=' || I*3);
...
END;
--------------------------------------------------------------------------------
--커서 CURSOR(질의결과가 1개 초과일경우 사용)
DECLARE
    NAME VARCHAR2(30); --단일값
BEGIN
    --SELECT 결과가 여러행이라서 ERR가 납니다.
    SELECT FIRST_NAME
    INTO NAME
    FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG'; --여러행

    DBMS_OUTPUT.PUT_LINE(NAME);
END; --에러 "exact fetch returns more than requested number of rows"
...
BEGIN
    ...
    LOOP
        ...
        EXIT WHEN X%NOTFOUND; --X커서의 더이상 읽을값이 없으면 TURE(X%NOTFOUND)
   ...
   DBMS_OUTPUT.PUT_LINE('데이터수:' || X%ROWCOUNT); --커서에서 읽은 데이터 수(%ROWCOUNT)
    
   CLOSE X; --커서닫음
   
END; --탈출(EXIT WHEN) 안하면 에러: buffer overflow
--------------------------------------------------------------------------------
--CURSOR 문제
