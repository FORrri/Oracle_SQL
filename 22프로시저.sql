...
CREATE OR REPLACE PROCEDURE NEW_JOB_PROC --이름이 같으면 자동으로 수정됨.
    (P_JOB_ID IN VARCHAR2,
     ...
     P_MAX_SALARY IN JOBS.MAX_SALARY%TYPE := 10000 --디폴트 매개변수(뒤에서부터 써야함)
    )
IS
BEGIN
...
END;
--
EXEC NEW_JOB_PROC('EXAMPLE', 'EXAPLE', 1000, 10000);
EXEC NEW_JOB_PROC('EXAMPLE'); --매개변수가 일치하지 않으면 X
EXEC NEW_JOB_PROC('SAMPLE', 'SAMPLE2'); -- DEFAULT매개변수가 있다면, 기본값으로 전달 됩니다.
...
BEGIN
    ...
    --예외처리 구문(예외를 만나면 예외문장이 실행됨)-지금은 예외가 없기 때문에 실행x
...
CREATE TABLE DAY_OF_SALES( --총매출 기록
    REGDATE DATE,
...
INSERT INTO SALES(SNO, NAME, TOTAL, PRICE) VALUES(2, '카페라떼', 2 ,2000);
INSERT INTO SALES(SNO, NAME, TOTAL, PRICE) VALUES(3, '콜드브루', 1 ,3000);
