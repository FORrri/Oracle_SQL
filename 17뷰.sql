--뷰
--제한적인 데이터를 쉽게 보기 위해서 미리 만들어논 가상테이블(가짜) 입니다.
--자주 사용되는 칼럼을 저장하면, 관리가 용이합니다.
--뷰는 물리적으로 데이터가 저장된 것을 아니고, 원본테이블을 기반으로 한 가상테이블이라고 생각하면 됩니다.
...
AS (
    SELECT E.EMPLOYEE_ID,
            FIRST_NAME || ' ' || LAST_NAME AS NAME,
            J.JOB_TITLE,
            J.MAX_SALARY, --(자동)수정: OR REPLACE 있어서
            J.MIN_SALARY,
            D.DEPARTMENT_NAME
    FROM EMPLOYEES E
    JOIN DEPARTMENTS D
