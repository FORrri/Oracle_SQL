--트리거: 트리거는 테이블에 부착한 형태로, AFTER, BEFORE트리거가 있습니다.
--AFTER은 DML문장이 타겟테이블이 실행된 이후에 동작하는 트리거
--BEFORE은 DML문장이 타켓테이블에 실행되기 이전에 동작하는 트리거
...
--TLB_USER에 UPDATE OR DELETE가 일어나면, 기존데이터를 백업.
...
BEGIN
    ...
    IF UPDATING THEN --UPDATE가 일어나면 TURE
        V := '수정';
    ELSIF DELETING THEN --DELETE가 일어나면 TURE
        V := '삭제';
    END IF;
